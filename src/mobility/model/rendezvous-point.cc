...
std::vector<Vector>
Connection::GetPoints (void)
{
  return m_waypoints;
}

RendezvousPoint*
Connection::GetTarget (void)
{
  return m_target;
}
...
RendezvousPoint::GetTimeLeftUntilClear (RendezvousPoint* rp)
{
  ...
  // With ravve's distance to rp function, this should be easy
  // This should probably be moved to MineMobilityModel
  return Seconds (1);
}

Vector
RendezvousPoint::GetPosition (void)
{
  return m_pos;
}
...
std::vector<Vector>
RendezvousPoint::ReverseVector (std::vector<Vector> vect)
{
